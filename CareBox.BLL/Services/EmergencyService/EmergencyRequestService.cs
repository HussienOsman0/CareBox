    ...
    public class EmergencyRequestService : IEmergencyRequestService
    {
        ...
        public async Task<EmergencyTrackingResponseDto> GetTrackingDetailsAsync(int userId, long requestId)
        {
            ...
            if (request.Status == RequestStatus.Pending)
                throw new Exception("Request is still pending. No provider has accepted yet.");
        ...
        public async Task<IEnumerable<ClientEmergencyRequestResponseDto>> GetClientEmergencyRequestsAsync(int userId, string? filter)
        {
            // 1. جلب العميل
            var client = await _unitOfWork.Clients.FindAsync(c => c.AppUserId == userId);
            if (client == null) throw new Exception("Client not found.");
            ...
            // 5. المابينج للـ DTO
            return requests.OrderByDescending(e => e.CreatedAt).Select(e => new ClientEmergencyRequestResponseDto
            {
                RequestId = e.RequestId,
